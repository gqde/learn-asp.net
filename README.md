...
---
# STACK COMPLETO: Todo lo que necesitas instalar

Te doy todo organizado por categoría con los comandos exactos para Linux y Windows.

---

## 1. RUNTIME Y SDK DE .NET

### Windows

```powershell
# Opción recomendada: Winget (viene con Windows 10/11)
winget install Microsoft.DotNet.SDK.8
winget install Microsoft.DotNet.SDK.9

# O alternativa: descargar desde https://dotnet.microsoft.com/download
```

### Linux (Ubuntu/Debian)

```bash
# Script oficial de Microsoft
...
```bash
# Instalar después de tener el SDK
...
dotnet tool install --global dotnet-outdated    # Verificar paquetes desactualizados
dotnet tool install --global dotnet-counters    # Monitoreo en tiempo real
dotnet tool install --global dotnet-trace       # Tracing de performance
dotnet tool install --global dotnet-dump        # Análisis de memory dumps
dotnet tool install --global dotnet-gcdump      # GC heap dumps
dotnet tool install --global dotnet-monitor     # Diagnostics en producción
dotnet tool install --global BenchmarkDotNet.Tool  # Microbenchmarks
dotnet tool install --global dotnet-format      # Formateo de código
dotnet tool install --global dotnet-grpc        # gRPC tools
dotnet tool install --global swashbuckle.aspnetcore.cli  # Swagger CLI
```

---

## 2. EDITOR / IDE (elige al menos uno)

### Visual Studio 2022 Community (Windows) — Recomendado

```powershell
winget install Microsoft.VisualStudio.2022.Community
```

**Workloads a seleccionar durante instalación:**
...
```
- ReSharper (JetBrains) — Opcional pero muy potente
- OzCode — Debugging avanzado
...
### Visual Studio Code (Windows y Linux) — Alternativa ligera
...
code --install-extension humao.rest-client               # Probar APIs
code --install-extension rangav.vscode-thunder-client     # Alternativa REST
code --install-extension muhammad-sammy.csharp
code --install-extension fernandoescolar.vscode-solution-explorer
code --install-extension kreativ-software.csharpextensions
code --install-extension adrianwilczynski.namespace-resolver
...
code --install-extensionstreemode.vscode-postman-collection
```

### JetBrains Rider (Windows y Linux) — Alternativa premium
...
```bash
# Linux: snap o JetBrains Toolbox
...
### SQL Server en Docker (Windows y Linux) — Recomendado para Linux
...
> SQLite se usa principalmente para testing con EF Core InMemory/SQLite provider.
...
```powershell
# Windows
winget install dbeaver.dbeaver                 # Multi-DB (gratis)
winget install Microsoft.SQLServerManagementStudio  # SSMS (solo SQL Server)
winget install AzureDataStudio                 # Azure Data Studio (cross-platform)
```
...
# Azure Data Studio
sudo snap install azure-data-studio
```
...
> Requiere WSL2 habilitado en Windows.
...
```bash
# Ya incluido con Docker Desktop (Windows) y docker-compose-plugin (Linux)
docker compose version
```

---

## 5. SERVICIOS EN DOCKER (todo el stack de infraestructura)

### `docker-compose.infrastructure.yml` — Guárdalo y ejecuta cuando lo necesites
...
services:
  ...
  # Redis (cache y SignalR backplane)
  redis:
  ...
  # Adminer (DB web admin - alternativa a DBeaver)
  adminer:
...
## 6. NODE.JS Y NPM (para frontend tooling)
...
**Paquetes NPM globales que usarás:**

```bash
npm install -g @microsoft/signalr    # SignalR client
...
winget install GitHub.GitHubDesktop  # GUI opcional
```
...
### Configuración inicial (ambos OS)
...
### Herramientas Git adicionales

```powershell
# Windows
winget install GitHub.cli              # GitHub CLI
winget install Fork.Fork               # Fork Git client (gratis trial)
```
...
## 8. HTTP CLIENTS / API TESTING

```powershell
# Windows
winget install Insomnia.Insomnia        # Alternativa a Postman
...
**También puedes usar:**
- **Bruno** (open source, git-friendly): `winget install Bruno.Bruno`
- **httpie**: `pip install httpie`
- **curl** (ya instalado en Linux, incluido en Windows 10+)
...
# Windows (ya viene en Docker, o usar Redis Desktop Manager)
winget install Redisant.RedisDesktopManager
```
...
```bash
# Linux
sudo apt install rabbitmq-server    # Si quieres instalarlo nativamente
# Mejor usar Docker
```

### Protocol Buffers Compiler (para gRPC)
...
# O usar el NuGet Grpc.Tools (ya incluido en el proyecto)
```

### Azure CLI (para deployment en Azure)
...
npm install -g sass                   # SCSS compiler
npm install -g live-server            # Servidor estático para pruebas
```

### Librerías que usarás en proyectos ASP.NET (vía Libman o npm):

```
- Bootstrap 5
- jQuery (para MVC validation)
- jquery-validation
...
```bash
# Ya incluidas en .NET SDK:
# - xUnit
# - NUnit
# - MSTest

# Paquetes NuGet que instalarás por proyecto:
...
## 12. SCRIPT DE INSTALACIÓN COMPLETA
...
Write-Host "=== Reinicia el sistema antes de continuar ==="

# === .NET TOOLS (después de reiniciar) ===
...
## 13. RESUMEN: QUÉ USAS EN CADA MÓDULO

| Módulos | Herramientas que necesitas |
|---|---|
| **1-2** (C#, .NET) | SDK .NET, VS/VS Code |
| **3** (HTML/CSS/JS) | VS Code, Node.js, navegador |
| **4-10** (MVC, Razor, Forms) | SDK .NET, VS, Bootstrap |
| **11-13** (EF Core) | SDK .NET, **SQL Server o PostgreSQL**, DBeaver |
| **14-18** (DI, Middleware, Config, Logging) | SDK .NET, **Seq** (docker) |
| **19-20** (Filtros, Auth cookies) | SDK .NET |
| **21-27** (Web API, REST, JWT, CORS) | SDK .NET, **Insomnia/Postman** |
| **28** (Razor Pages) | SDK .NET |
| **29** (SignalR) | SDK .NET, **Redis** (docker) |
| **30-32** (Blazor) | SDK .NET, Node.js (npm para SignalR JS) |
| **33** (Minimal APIs) | SDK .NET, Insomnia |
| **34** (Caching) | SDK .NET, **Redis** (docker) |
| **35** (Background Services) | SDK .NET |
| **36** (File Upload) | SDK .NET, **Azure Blob** o disco local |
| **37-39** (Health Checks, Rate Limiting, Output Cache) | SDK .NET |
| **40** (gRPC) | SDK .NET, **protobuf compiler** |
| **41-43** (Clean Arch, CQRS, DDD) | SDK .NET, DBeaver |
| **44-45** (Microservices, Message Queues) | SDK .NET, **Docker**, **RabbitMQ** (docker), **Redis** (docker) |
| **46** (GraphQL) | SDK .NET, HotChocolate |
| **47** (Identity, OAuth) | SDK .NET, **Postman** |
| **48** (Testing) | SDK .NET, **Playwright** |
| **49** (CI/CD) | **Git**, **Docker**, **Azure CLI**, GitHub |
| **50** (Performance) | **BenchmarkDotNet**, **dotnet-trace**, **dotnet-counters** |
...
|---|---|
| .NET SDK | **Gratis** |
| Visual Studio Community | **Gratis** (individual/open source) |
| VS Code | **Gratis** |
| SQL Server Express | **Gratis** (limit: 10GB) |
| PostgreSQL | **Gratis** |
| Docker Desktop | **Gratis** (individual/small business) |
| Redis | **Gratis** |
| RabbitMQ | **Gratis** |
| DBeaver Community | **Gratis** |
| Git | **Gratis** |
| Postman / Insomnia | **Gratis** (plan básico) |
| Node.js | **Gratis** |
| GitHub | **Gratis** (repos públicos) |
| Seq (logging) | **Gratis** (hasta 1 usuario) |

**Costo total: $0** — Todo es gratuito para aprendizaje y desarrollo individual.

---

## 15. VERIFICACIÓN FINAL

Después de instalar todo, ejecuta esto para verificar:
...
echo "=== Ejecutar infraestructura ==="
# Copia el docker-compose.infrastructure.yml y ejecuta:
docker compose -f docker-compose.infrastructure.yml up -d
docker compose -f docker-compose.infrastructure.yml ps
```

Si todo muestra versiones y los contenedores arrancan, **estás listo para empezar el Módulo 1**.
